...
class Timer:
    @classmethod
    async def start_class_timer(cls, sch_class_id: int):
        now = datetime.now()
        class_ = await Class.get_or_none(id=sch_class_id)

        if class_ and not class_.start_time:
            # Записываем время старта в таблицу `Class`
            await Class.filter(id=sch_class_id).update(start_time=now)
            end_time = now + timedelta(minutes=20)

            # Запускаем фоновую задачу для отслеживания таймера
            asyncio.create_task(cls.timer_notifications(sch_class_id, end_time))

    @classmethod
    async def timer_notifications(cls, sch_class_id: int, end_time: datetime):
        ...
        for minutes in time_intervals:
            ...
            users = await User.filter(sch_class=sch_class_id).values_list("tg_id", flat=True)
            ...
            if minutes > 0:
                ...
                await Class.filter(id=sch_class_id).update(start_time=None)
