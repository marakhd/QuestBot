...
class Timer:
    async def start_class_timer(self, sch_class: int):
        now = datetime.now()
        class_ = await Class.get_or_none(id=sch_class)

        if class_ and not class_.start_time:
            # Записываем время старта в таблицу `Class`
            await Class.filter(id=sch_class).update(start_time=now)
            end_time = now + timedelta(minutes=20)

            # Запускаем фоновую задачу для отслеживания таймера
            asyncio.create_task(self.timer_notifications(sch_class, end_time))

    async def timer_notifications(self, sch_class: int, end_time: datetime):
        ...
        for minutes in time_intervals:
            ...
            users = await User.filter(sch_class=sch_class).values_list("tg_id", flat=True)
            ...
            if minutes > 0:
                ...
                await Class.filter(id=sch_class).update(start_time=None)
